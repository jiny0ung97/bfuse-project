...
extern "C" __global__ void __launch_bounds__(128) softmax_shared_1036(float* __restrict__ T_softmax_norm, float* __restrict__ data) {
  ...
  __shared__ float red_result[1+256];//
  __shared__ float T_softmax_maxelem[1+256];//
  float normal_reduce_temp0_1[1];
  __shared__ float red_result_1[1+256];//
  __shared__ float T_softmax_expsum[1+256];//
...
extern "C" __global__ void __launch_bounds__(128) softmax_shared_1036_copy(float* __restrict__ T_softmax_norm, float* __restrict__ data) {
  float normal_reduce_temp0[1];
  __shared__ float red_result[1+256];//
  __shared__ float T_softmax_maxelem[1+256];//
  float normal_reduce_temp0_1[1];
