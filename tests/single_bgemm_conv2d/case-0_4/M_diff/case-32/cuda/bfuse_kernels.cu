extern "C" __global__ __launch_bounds__(152) void bgemm_0_conv2d_4_fused_bfuse(float *__restrict bgemm_0_A_, float *__restrict bgemm_0_B_, float *__restrict bgemm_0_T_batch_matmul_NT_, float *__restrict conv2d_4_conv2d_nchw_, float *__restrict conv2d_4_data_, float *__restrict conv2d_4_kernel_)
{
  ...
  if (((int)blockIdx.x < 393216) && ((int)blockIdx.x % 6 >= 0) && ((int)blockIdx.x % 6 < 1))
  {
    NewBlockIdx_ = int((int)blockIdx.x / 6) * 1 + ((int)blockIdx.x % 6 - 0);
    KernelID_  = 0;
    gridDim_x_ = 8;
    gridDim_y_ = 8;
    gridDim_z_ = 1024;
  ...
  else if (((int)blockIdx.x < 393216) && ((int)blockIdx.x % 6 >= 1) && ((int)blockIdx.x % 6 < 6))
  {
    NewBlockIdx_ = int((int)blockIdx.x / 6) * 5 + ((int)blockIdx.x % 6 - 1);
    KernelID_  = 1;
    gridDim_x_ = 3;
    gridDim_y_ = 57;
    gridDim_z_ = 2048;
  ...
  else if ((int)blockIdx.x >= 393216 && (int)blockIdx.x < 393216)
  {
    NewBlockIdx_ = (int)blockIdx.x - 393216 + 65536;
    KernelID_  = 0;
    gridDim_x_ = 8;
    gridDim_y_ = 8;
    gridDim_z_ = 1024;
  ...
  else if ((int)blockIdx.x >= 393216 && (int)blockIdx.x < 415744)
  {
    NewBlockIdx_ = (int)blockIdx.x - 393216 + 327680;
    KernelID_  = 1;
    gridDim_x_ = 3;
    gridDim_y_ = 57;
    gridDim_z_ = 2048;
