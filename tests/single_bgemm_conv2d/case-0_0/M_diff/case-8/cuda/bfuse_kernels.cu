extern "C" __global__ __launch_bounds__(64) void bgemm_0_conv2d_0_fused_bfuse(float *__restrict bgemm_0_A_, float *__restrict bgemm_0_B_, float *__restrict bgemm_0_T_batch_matmul_NT_, float *__restrict conv2d_0_conv2d_nchw_, float *__restrict conv2d_0_data_, float *__restrict conv2d_0_kernel_)
{
  ...
  if (((int)blockIdx.x < 589824) && ((int)blockIdx.x % 9 >= 0) && ((int)blockIdx.x % 9 < 1))
  {
    NewBlockIdx_ = int((int)blockIdx.x / 9) * 1 + ((int)blockIdx.x % 9 - 0);
    KernelID_  = 0;
    gridDim_x_ = 8;
    gridDim_y_ = 8;
    gridDim_z_ = 1024;
  ...
  else if (((int)blockIdx.x < 589824) && ((int)blockIdx.x % 9 >= 1) && ((int)blockIdx.x % 9 < 9))
  {
    NewBlockIdx_ = int((int)blockIdx.x / 9) * 8 + ((int)blockIdx.x % 9 - 1);
    KernelID_  = 1;
    gridDim_x_ = 55;
    gridDim_y_ = 11;
    gridDim_z_ = 3072;
  ...
  else if ((int)blockIdx.x >= 589824 && (int)blockIdx.x < 589824)
  {
    NewBlockIdx_ = (int)blockIdx.x - 589824 + 65536;
    KernelID_  = 0;
    gridDim_x_ = 8;
    gridDim_y_ = 8;
    gridDim_z_ = 1024;
  ...
  else if ((int)blockIdx.x >= 589824 && (int)blockIdx.x < 1924096)
  {
    NewBlockIdx_ = (int)blockIdx.x - 589824 + 524288;
    KernelID_  = 1;
    gridDim_x_ = 55;
    gridDim_y_ = 11;
    gridDim_z_ = 3072;
