...
{
  // Impl: 1*MinOption : n*MinOption for each iter
...
//---------------------------------------------------------------------------
tuple<string, map<string, string>, GridDim, BlockDim> coarseInterleaveWithoutSMPattern2(vector<string> &Kernels, map<string, KernelInfo> &KernelInfoMap)
{
  // Impl: 1*N : M*N for each iter
  ...
  for (auto &KName : Kernels) {
    auto &KInfo        = KernelInfoMap.at(KName);
    int BlockRatio     = int(BlockRatios.at(KName) / MinRatio);
    BlockRatio         = BlockRatio < M ? BlockRatio*N : M*N;
    int Quot           = int(KInfo.GridDim_.size() / BlockRatio);
    BlockRatios[KName] = BlockRatio;
